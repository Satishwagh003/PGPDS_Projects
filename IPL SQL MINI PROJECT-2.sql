...
-- Note 1. use the performance dtls column from ipl player to get the total number of
-- wickets
-- 2. Do not use the limit method because it might not give appropriate results when
-- players have the same number of wickets 3. Do not use joins in any cases.
-- 4. Display the following columns teamn name, player name, and player role.

SELECT Team_name, Player_name, Player_role
FROM
    (
	SELECT PLAYER_ROLE, Team_name, ipl_player.PLAYER_ID, PLAYER_NAME,
        ...
        FROM ipl_player, ipl_team_players, ipl_team
	    where
            ipl_player.PLAYER_ID = ipl_team_players.PLAYER_ID
...
COUNT(if((mat.TEAM_ID1=bdg.BID_TEAM and mat.TOSS_WINNER = 1) or (mat.TEAM_ID2=bdg.BID_TEAM and mat.TOSS_WINNER = 2),1,NULL))
/
COUNT(*)*100 AS Toss_percentage
FROM ipl_match mat JOIN ipl_match_schedule ms
USING(MATCH_ID)
INNER JOIN ipl_bidding_details bdg
USING(SCHEDULE_ID)
INNER JOIN ipl_bidder_details
USING(BIDDER_ID)
...
-- 12)  find the IPL season which has min duration and max duration. Output columns should be like the below:
 ...
 --         Tournment ID, Tourment name, Duration column, Duration 
...
WHERE Duration_column IS NOT NULL;
 ...
 -- 13)  Write a query to display to calculate the total points month-wise for the 2017 bid year. sort the results based on total points in descending order and month-wise in ascending order. Note: Display the following columns:
...
SELECT DISTINCT a.Bidder_ID, c.BIDDER_NAME, month(BID_DATE), Year(BID_DATE), TOTAL_POINTS
FROM ipl_bidder_points a JOIN ipl_bidding_details b
USING(BIDDER_ID)
JOIN ipl_bidder_details c
USING(BIDDER_ID)
WHERE Year(BID_DATE) = 2017
ORDER BY TOTAL_POINTS DESC , month(BID_DATE) ASC ;

 -- 14)  Write a query for the above question using sub queries by having the same constraints as the above question.alter
 
 select bidder_id,
     (
     select bidder_name
     from ipl_bidder_details
	 where ipl_bidder_details.bidder_id = ipl_bidding_details.bidder_id) as bidder_name, year(bid_date) as 'year', monthname (bid_date) as 'month',
     (
          select total_points
          from ipl_bidder_points
          where ipl_bidder_points.bidder_id = ipl_bidding_details.bidder_id
	 )    as total_points
 ...
 -- 15) 15. Write a query to get the top 3 and bottom 3 bidders based on the total bidding points for the 2018 bidding year. Output columns should be.

-- like Bidder td, Ranks Total points, Highest 3 Bidders-> columns contains name of Lowest 3 Bidders columns contains name of bidder;
     
select * from
    (
    select pts.BIDDER_ID, pts.TOTAL_POINTS, dense_rank() over(order by pts.TOTAL_POINTS desc) as Ranks, bdr.BIDDER_NAME,
		'Highest_3_Bidders' as 'Highest/Lowest_3_Bidders'
	from ipl_bidder_points pts inner join ipl_bidder_details bdr
	on
   pts.BIDDER_ID = bdr.BIDDER_ID
   )temp
   where Ranks<4
   union all
(
select * from (
select pts.BIDDER_ID, pts.TOTAL_POINTS, rank() over(order by pts. TOTAL_POINTS) as Ranks2, bdr.BIDDER_NAME, "Lowest_3_Bidders"
from ipl_bidder_points pts inner join ipl_bidder_details bdr on pts.BIDDER_ID = bdr.BIDDER_ID) temp2
where Ranks2<4);
     
-- 16) two tables called Student details and Student details backup.
